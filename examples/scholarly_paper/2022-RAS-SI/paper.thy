...
title*[tit::title]\<open>Modeling and Analysing Cyber-Physical Systems in \<^holcsp>\<close>
...
author*[lina,email="\<open>paolo.crisafulli@irt-systemx.fr\<close>",affiliation="\<open>IRT SystemX, Palaiseau\<close>"]\<open>Paolo Crisafulli\<close>
               
abstract*[abs, keywordlist="[\<open>Cyber-Physical Systems\<close>,\<open>Autonomous Cars\<close>,\<open>Safety-Critical Systems\<close>,
                             \<open>Process-Algebra\<close>,\<open>Concurrency\<close>,\<open>Proof-based Verification\<close>]"]
\<open>  Modelling and Analysing Cyber-Physical Systems (CPS) is a challenge 
   for Formal Methods and therefore a field of active research. It is 
   characteristic for CPS that models comprise aspects of Newtonian 
   Physics appearing in system environments, the difficulties of their 
   discretisation, the problems of communication and interaction between 
   actors in this environment as well as calculations respecting 
   time-bounds.
   We present a novel approach to address these problems developed within 
   an IRT SystemX project for industrial partners involved in the 
   Autonomous Car Domain. Based on  \<^holcsp>, a conservative extension of 
   Higher-order Logic (HOL) with the language of “Concurrent Sequential 
   Processes” (CSP), we model time, physical evolution, “scenes” (global 
   states) and “scenarios” (traces) as well as the interaction of “actors” 
   (cars, pedestrians, traffic lights) inside this framework. In particular, 
   infinitesimal time is modelled by infinite internal choice, emphasizing 
   that DE’s can be viewed as the set of possible samplings.
   For a special case of the modelling framework, we use the power of 
   interactive theorem proving in Isabelle/HOL for the analysis of a 
   particular safety property for Autonomous Cars: In a one-lane scenario, 
   where each car follows the Responsibility-Sensitive Safety (RSS)-driving 
   strategy,
% (https://www.mobileye.com/responsibility-sensitive-safety/)
   no  collision will occur. The proof gives rise to a number of optimisations as 
   well as a partitioning of abstract test cases.
\<close>
...
\<^item> the discretising view of continuous observables in 
  systems behaviour (sampling into 'snapshots'),
\<^item> and computations in sensors and control-systems that 
  typically have to meet hard time-bounds.

Our understanding of 'analysis' of CPS involves:
\<^item> refinement relations between models: saying that some particular model is 
  \<^emph>\<open>better\<close> in some to be defined sense than another,
\<^item> the refinement of models to executable code,
\<^item> techniques to establish properties by means of mechanizable,
  formal analysis, \<^ie> techniques involving test and proof. 
...
\<^item> Communicating Sequential Processes (\<^csp>), which is a language
  ...
  \<^csp>'s rich theory comprises denotational, operational and algebraic semantic 
  facets and has influenced programming languages such as Crystal and
  most notably Golang @{cite "donovan2015go"}.
% \<^csp> has been applied in 
...
\<^item> Isabelle is a flexible system framework allowing for logically safe extensions, 
  which comprise both theories as well as implementations for code-generation, 
  documentation, and specific support for a variety of formal methods. 
  It can be seen as an IDE for formal methods. 
\<^item> \<^isabelle> is an instance of Isabelle with a theory, automated proof support, and libraries
  for \<^emph>\<open>Higher-order Logic (\<^hol>)\<close>. The libraries, which are constructed as logically safe
...
\<^item> refinement notions for CPS: it turns out, that in many cases \<^csp> standard refinement notions
...
In the last part of this paper, we formalize, correct, and generalize this analysis by
a machine-checked proof in \<^isabelle> and \<^holcsp> --- to our knowledge, this is the first formal
proof of a driving strategy at all. Our formal analysis also allows for substantial improvements
of the strategy (smaller safety-distances by same safety-goals) and work-out ways to extract
test-data for system integration tests from the formal proofs.
\<close>
